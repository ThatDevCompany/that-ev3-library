...
export class Device {
	...
	connect(
		...
		propertyConstraints?: { [propertyName: string]: any }
	) {
		const nameRegex = nameConvention ? new RegExp(nameConvention) : undefined

		const deviceSearchDir = path.join(
			this._overrideSysClassDir || this._sysClassDir,
			driverName
		)
	...
	readProperty(property: string, deviceRoot?: string): string {
		if (!deviceRoot && !this._connected) {
			throw new Error(
				'You must be connected to a device before you can read from it. This error probably means that the target device was not found.'
			)
		}
		...
		try {
			...
			console.error(e)
			throw new Error(
				'There was an error while reading from the property file ' +
					propertyPath
			)
		}
	}
	...
	setProperty(property: string, value: string) {
		if (!this._connected) {
			throw new Error(
				'You must be connected to a device before you can write to it. This error probably means that the target device was not found.'
			)
		}
		...
		try {
			...
			console.error(e)
			throw new Error(
				'There was an error while writing to the property file ' + propertyPath
			)
		}
	}
	...
	protected updateClock() {
		...
		if (this._handlers.length && !this._clock) {
			this._clock = setInterval(() => this.tick(), this._clockSpeed)
